...
mysql_get_user_option() {
    ...
    then
        ...
        local defaults_extra_file="$(echo "$@"|grep -o -P "\s-defaults-extra-file\s*=\s*(?:[\S]+|\"[\s\S]+?\"|'[\s\S]+?')"|tail -n 1)"
        if [[ -z "$no_defaults" ]]
        then
...
docker_create_db_directories() {
	...
	if [ "$user" = "0" ]; then
		...
		find "${!dirs[@]}" \! -user $proc_user -exec chown --no-dereference $proc_user '{}' +
	fi
}
...
_main() {
	...
	if [ "$1" = 'mysqld' ] && ! _mysql_want_help "$@"; then
        ...
        local proc_user="$(mysql_get_user_option $@)"
        #If container is started as root user, restart as dedicated mysql user
		if [ "$(id -u)" = "0" ]; then
			mysql_note "Switching to dedicated user '$proc_user'"
			exec gosu $proc_user "$BASH_SOURCE" "$@"
		fi
...
proc_user="$(mysql_get_user_option $@)"
if [[ -z "$proc_user" ]]
then
    proc_user="mysql"
fi
